 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-08-04 10:42:02
-->
<template>
  ...
  methods: {
    ...
    hdToAnalyse (item) {
      ...
      if (item.analyseScore > 0) {
        this.$router.push({ name: 'WeekDetailDouyin', query: { id: item.id } })
      } else {
        this.$message.warning('还在分析中，请稍后')
      }

      // this.$router.push({ name: 'WeekDetailToutiao', query: { id: item.id } })
    },
    goDelete () {
    ...
    confirmDelete () {
      ...
      this.$http.post(this.$api.delAnalyseAccountToutiao, { ids: ids })
        .then(res => {
          ids.forEach((item) => {
    ...
    // 获取任务列表
    getTaskList () {
      this.$http.post(this.$api.findAnalyseAccountToutiao, this.ruleForm)
        .then(res => {
