 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-06-02 10:30:18
-->
<template>
      ...
      <div class="add-main flex-cloumn-cen">
        ...
        <div class="add-main-center">
          <div class="add-task-duration">
            ...
            <p class="tips"
               v-if="this.duration==='1'">为您提供最近一周微博数据报告</p>
            <p class="tips"
               v-else>为您提供最近一月微博数据报告</p>
  ...
  methods: {
    ...
    startAnalyse () {
      ...
      if (this.selectItem.uid) {
        let obj = {
          accountId: this.selectItem.uid,
          dateType: this.ruleForm.dateType,
          startDate: this.cycle[0],
          endDate: this.cycle[1]
        }
        this.$http.post(this.$api.addAnalyseAccount, obj)
          .then(res => {
    ...
    // 获取搜索记录
    getRecord () {
      this.$http.get(this.$api.findQueryWb)
        .then(res => {
    ...
    // 删除搜索记录
    deleteRecord () {
      this.$http.get(this.$api.delQueryLogWb)
        .then(res => {
          this.record = []
