 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-08-04 09:50:02
-->
<template>
  <div class="interaction content-box">
    ...
    <div class="pub-monitor-main">
      <douyin-add v-if="this.activeTab==='createTask'"
                       @changeTab="tabsAll"></douyin-add>
      <douyin-list v-if="this.activeTab==='taskList'"></douyin-list>
    </div>

  </div>
</template>
<script>
import douyinAdd from './DouyinAdd'
import douyinList from './DouyinList'
export default {
  components: {
    douyinAdd,
    douyinList
  ...
  methods: {
    ...
    tabsAll (name, level) {
      ...
      this.$router.replace({ name: 'DouyinAnalysis', query: { tab: name } })
      this.activeTab = name
    }
  },
  created () {
