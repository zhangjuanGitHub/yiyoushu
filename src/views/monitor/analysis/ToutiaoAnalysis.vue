 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-07-15 17:37:07
-->
<template>
  <div class="interaction content-box">
    ...
    <div class="pub-monitor-main">
      <toutiao-add v-if="this.activeTab==='createTask'"
                       @changeTab="tabsAll"></toutiao-add>
      <toutiao-list v-if="this.activeTab==='taskList'"></toutiao-list>
    </div>

  </div>
</template>
<script>
import toutiaoAdd from './ToutiaoAdd'
import toutiaoList from './ToutiaoList'
export default {
  components: {
    toutiaoAdd,
    toutiaoList
  ...
  methods: {
    ...
    tabsAll (name, level) {
      ...
      this.$router.replace({ name: 'ToutiaoAnalysis', query: { tab: name } })
      this.activeTab = name
    }
  },
  created () {
