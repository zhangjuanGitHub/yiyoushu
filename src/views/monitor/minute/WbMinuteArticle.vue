<!--
 * @Author: zhangjuan
 * @Date: 2021-05-31 14:28:14
 * @LastEditors: zhangjuan
 * @LastEditTime: 2021-06-03 11:06:46
-->
<template>
  <div class="minute-article">
    ...
    <div class="minute-content">
      ...
      <div class="minute-introduce">可对微博进行实时监控。为您提供分钟级的点赞数、评论数、转发数监测，掌握微博文章热度走向。</div>
        ...
        <el-form :model="ruleForm"
          ...
          <div class="link-box flex-ali-center"
            ...
            <div class="left-image">
              <img :src="articleData.avatarHd"
                   alt="">
            </div>
            <div class="right-detail">
              <div class="right-top">
                <span class="name">{{articleData.screenName}}</span>
                <span class="send-time">日常发文时间:</span>
                <span class="send-detail">{{articleData.pubtime}}</span>
              </div>
              <div class="right-bottom">
                <p class="lin-clamp-1">{{articleData.statusTitle}}</p>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    queryLink (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post(this.$api.serachWbByurl, { url: this.ruleForm.link })
            .then(res => {
              if (res.data.data) {
    ...
    startAngle (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let obj = {
            dayCount: this.ruleForm.time,
            url: this.articleData.url,
            uid: this.articleData.uid,
            mid: this.articleData.mid,
            rawText: this.articleData.rawText,
            pubtime: this.articleData.pubtime,
            screenName: this.articleData.screenName,
            profileImageUrl: this.articleData.avatarHd,
            statusTitle: this.articleData.statusTitle
          }
          this.$http.post(this.$api.startWbArticle, obj)
            .then(res => {
