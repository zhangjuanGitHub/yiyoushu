<!--
 * @Author: zhangjuan
 * @Date: 2021-05-31 14:27:41
 * @LastEditors: zhangjuan
 * @LastEditTime: 2021-06-04 17:57:00
-->
<template>
  <div class="minute-account">
    ...
    <div class="minute-content">
      ...
      <div class="minute-introduce">可对微博进行预约监控。为您提供分钟级的点赞数、评论数、转发数监测，掌握微博文章热度走向。</div>
      <div class="minute-from">
        <el-form :model="ruleForm"
          ...
          <el-form-item prop="dayCount"
            ...
            <div class="link-line minute-wb-days">
              ...
              <el-select v-model="ruleForm.dayCount"
                         size="small">
...
export default {
  ...
  methods: {
    // 获取回显数据
    getAccount () {
      this.$http.get(this.$api.wbMinAccount, { params: { id: this.echoId } })
        .then(res => {
          if (res.data.code === 200) {
            ...
            this.ruleForm.keyword = res.data.data.screenName
    ...
    queryLink (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.accountData = []
          this.$http.post(this.$api.searchWbByName, { keyword: this.ruleForm.keyword })
            .then(res => {
    ...
    startSave (formName) {
      if (this.ruleForm.stardate && this.ruleForm.enddate && this.acticleList.length > 0) {
        let obj = {
          screenName: this.acticleList[0].screenName,
          profileImageUrl: this.acticleList[0].profileImageUrl,
          id: this.acticleList[0].id,
          uid: this.acticleList[0].uid,
          dayCount: this.ruleForm.dayCount,
          startTime: this.ruleForm.startTime,
          endTime: this.ruleForm.endTime
        }
        this.$http.post(this.$api.updateWbMin, obj)
          .then(res => {
            if (res.data.code === 200) {
              ...
              this.$router.push({ name: 'MinuteList', query: { tab: 'wb' } })
            }
          }).catch(() => { })
      } else {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.ruleForm.stardate && this.ruleForm.enddate && this.acticleList.length > 0) {
            ...
            this.$http.post(this.$api.startWbAccount, obj)
              .then(res => {
                if (res.data.code === 200) {
                  this.$message.success('操作成功!')
                  this.accountData = []
    ...
    // 选中
    addList (item) {
      this.$http.post(this.$api.checkWbSel, { uid: item.uid })
        .then(res => {
          if (res.data.code === 200) {
