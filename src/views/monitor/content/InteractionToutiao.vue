 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-08-03 13:41:45
        ...
        <el-table :data="tableData"
          ...
          <el-table-column prop="nickname"
            ...
            <template slot-scope='scope'>
              <div class="account-infor flex-ali-center cursor" @click="getWb(scope.row)">
          ...
          <el-table-column prop="repostsSum"
                           label="转发总数"
                           width="145">
          </el-table-column>
...
const getSort = new Map()
  .set('articleUp', 'asc')
  .set('articleDo', 'desc') // 发文数
  .set('oldLikeUp', 'asc')
  .set('oldLikeDo', 'desc') // 点赞
  .set('interUp', 'asc')
  .set('interDo', 'desc') // 互动指数
  .set('commentUp', 'asc')
  .set('commentDo', 'desc') // 评论总数
  .set('repostsUp', 'asc')
  .set('repostsDo', 'desc') // 转发总数
  .set('followUp', 'asc')
  .set('followDo', 'desc') // 关注总数
export default {
  data () {
    return {
      activeTab: 'wb',
  ...
  methods: {
    ...
    getTableData () {
      ...
      this.$http.post(this.$api.findInteractivityListToutiao, this.ruleForm)
        .then(res => {
    ...
    getWb (row) {
      console.log(row)
      this.$router.push({ name: 'ReleaseRulesToutiao', query: { id: row.accountId } })
