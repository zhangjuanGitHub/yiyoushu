 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-06-01 17:54:33
-->
<template>
          ...
          <el-table-column prop="status_title"
            ...
            <template slot-scope="scope">
              <div v-if="scope.row.url"
                   ...
                   v-html="scope.row.status_title">
                <!-- {{scope.row.status_title}} -->
              </div>
              <div v-else
                   v-html="scope.row.status_title"></div>
            </template>
          </el-table-column>
...
export default {
  ...
  created () {
    ...
    this.ruleForm.uid = this.$route.query.id
  },
  methods: {
    // 打开全文链接
    openUrl (item) {
    ...
    // 获取表格信息
    searchTable () {
      this.$http.post(this.$api.findArticlePertinenceList, this.ruleForm)
        .then(res => {
    ...
    pieCharts () {
      ...
      this.$http.post(this.$api.findArticlePertinencePieChart, params)
        .then(res => {
    ...
    drawLine () {
      ...
      this.$http.post(this.$api.findArticlePertinenceChart, params)
        .then(res => {
