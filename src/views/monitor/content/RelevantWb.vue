...
export default {
  ...
  methods: {
    ...
    // 获取表格信息
    getTableData () {
      this.$http.post(this.$api.findAccountPertinenceWb, this.ruleForm)
        .then(res => {
    ...
    openInvolved (row, level) {
      ...
      let route = this.$router.resolve({ name: 'RelevantDetailWb', query: { id: row.uid, type: level, publishTime: this.ruleForm.publishTime } })
