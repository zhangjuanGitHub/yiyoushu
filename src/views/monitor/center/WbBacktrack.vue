<!--
 * @Author: zhangjuan
 * @Date: 2021-05-25 15:37:47
 * @LastEditors: zhangjuan
 * @LastEditTime: 2021-06-03 17:30:14
-->
<template>
  <div>
    <div class="search-material-wrap flex-cloumn-all">
      <p class="search-back-title">数据回溯-微博</p>
      <el-button type="primary" plain
                  @click="openAddVisible"><i class="el-icon-plus"></i>添加微博号</el-button>
      ...
      <el-table :data="backData"
                style="width: 400px; border: 1px solid #dddddd">
        <el-table-column label="微博号名称"
                          prop="screen_name"
                          show-overflow-tooltip>
          <template slot-scope="scope">
            <div class="content-box-avatar flex-ali-center">
              <img :src="scope.row.avatar_hd" alt="">
              <p>{{scope.row.nickname}}</p>
            </div>
          </template>
        </el-table-column>
        <!-- <el-table-column label="微博号"
                          prop="alias"
                          width="440">
        </el-table-column> -->
              ...
              <template>
                <div class="add-box">
                  <div class="search-box">
                    <el-input placeholder="请输入微博号名称或微博号"
                              v-model="keyword">
                    </el-input>
                    ...
                    <div class="show-box">
                      ...
                      <div class="content-box">
                        <div class="flex-bwt-center alone cursor"
                          ...
                          <div class="content-box-avatar flex-ali-center">
                            <img :src="item.avatar_hd || undefinedUrl" alt="">
  ...
  methods: {
    ...
    queryKeyword () {
      if (this.keyword) {
        ...
        this.$message.warning('请输入微博账号名称')
      }
    },
    // 根据Url搜索
    queryUrl () {
    ...
    confirmToSubmit () {
      if (this.activeName === 'first') { // 搜索账号添加
        ...
        this.urlData.map(item => {
          // this.backData.push(item)
          this.backData.push({
            avatar_hd: item.avatarHd,
            nickname: item.screenName,
            accountId: item.uid
          })
        })
        this.dateForm.url = ''
    ...
    deleteBackItem (item) {
      this.backData.forEach((val, index) => {
        if (item.accountId === val.accountId) {
