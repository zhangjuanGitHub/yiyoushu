 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-08-03 10:18:15
 * @LastEditors: MaiChao
      ...
      <el-form :inline="true"
        ...
        <el-form-item label="">
          <el-date-picker v-model="pageBean.publishTime"
                          ...
                          end-placeholder="结束日期" size="small" style="width:240px;">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="">
          <el-input v-model="pageBean.keyword"
                    placeholder="请输入关键词搜索" size="small"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary"
                     @click="getSearchList" size="small">查询</el-button>
          <el-button type="warning"
                     @click="exportExcel" size="small">导出</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="wx-warp-content">
      <div class="interaction-table">
        <el-table :data="tableData"
          ...
          <el-table-column prop="last_publish_time"
                           label="最后发布时间"
                           width="120">
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
        ...
        .then(res => {
          ...
          if (res.data.data.count > 0) {
            ...
            this.tableData.forEach(item => {
              item.last_publish_time = item.last_publish_time.slice(0, 10)
            })
          } else {
    ...
    getDouyin (row) {
      ...
      this.$router.push({ name: 'ReleaseRulesDouyin', query: { id: row.user_id } })
    },
    formatterType (row) {
