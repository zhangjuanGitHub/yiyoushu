          ...
          <el-form :inline="true"
            ...
            <div class="flex-ali-center">
              ...
              <el-form-item>
                ...
                <el-button size="small"
                           @click="applyList(1)">添加搜索公众号</el-button>
              </el-form-item>
            </div>
            <div class="flex-ali-center">
              <el-form-item>
              ...
              <el-form-item>
                ...
                <el-button size="small"
                           @click="applyList(2)">添加收藏公众号</el-button>
              </el-form-item>
            </div>
          </el-form>
        </div>
...
export default {
  ...
  methods: {
    ...
    // 确定添加
    defineAdd () {
      this.$http.get(this.$api.addAccount, { params: { account: this.account.biz, accountName: this.account.nickname, type: this.type } })
        .then(res => {
    ...
    // 取消收藏
    defineDeldte () {
      this.$http.get(this.$api.defineDeldte, { params: { account: this.account.biz, type: this.type } })
        .then(res => {
          if (res.data.code === 200) {
            this.$message.success('操作成功!')
    ...
    // 获取表格数据
    geListData () {
      this.$http.post(this.$api.userListData, this.ruleForm)
        .then(res => {
          if (res.data.data.list.length > 0) {
            this.tableData = res.data.data.list
            this.limitCount = res.data.data.limitCoun
          }
        })
        .catch(() => { })
    },
    ...
    // 确定删除
    deleteTeams () {
      this.$http.get(this.$api.itemDelete, { params: { biz: this.deleteItem.biz, rankingUserId: this.ruleForm.rankingUserId } })
        .then(res => {
          this.$message.success('操作成功!')
          this.overDelete()
