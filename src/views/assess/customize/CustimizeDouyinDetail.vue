 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-08-03 16:20:12
-->
<template>
  <div class="contents">
            ...
            <div class="flex-ali-center">
              ...
              <el-form-item>
                <el-button size="small"
                           ...
                           type="primary">筛选</el-button>
                <el-button size="small"
                           @click="cleanSearch">重置</el-button>
                <el-button size="small"
                           type="warning"
        ...
        <div class="table-box">
          <el-table :data="tableData"
            ...
            <el-table-column prop="screenName"
                           ...
                           width="220">
            <template slot-scope='scope'>
              <div class="account-msg-box flex-ali-center cursor flex-ali-center2"
          ...
          <el-table-column prop="functionType"
                           label="职能">
            <template slot-scope='scope'>
              <p v-if="scope.row.functionType == 'other'">其他</p>
              <p v-else>{{scope.row.functionType}}</p>
            </template>
          </el-table-column>
              ...
              <template>
                <div class="add-box">
                  <div class="search-box">
                    <el-input placeholder="请输入抖音昵称或者抖音号"
                              v-model="keyword">
                    </el-input>
...
export default {
  ...
  methods: {
    ...
    // 确定添加
    defineAdd () {
      this.$http.get(this.$api.addAccount, { params: { account: this.account.userId, accountName: this.account.screenName, type: this.type } })
        .then(res => {
    ...
    defineDeldte () {
      ...
      this.$http.get(this.$api.defineDeldte, { params: { account: this.account.userId, type: this.type } })
        .then(res => {
          if (res.data.code === 200) {
            this.$message.success('操作成功!')
    ...
    // 获取表格数据
    geListData () {
      this.$http.post(this.$api.userToutiaoListData, this.ruleForm)
        .then(res => {
    ...
    // 确定删除
    deleteTeams () {
      this.$http.get(this.$api.itemDelete, { params: { biz: this.deleteItem.userId, rankingUserId: this.ruleForm.rankingUserId } })
        .then(res => {
          this.$message.success('操作成功!')
          this.overDelete()
    ...
    queryKeyword () {
      if (this.keyword) {
        this.$http.post(this.$api.keywordToutiao, { keyword: this.keyword })
          .then(res => {
      ...
      if (Number(item.is_collect) === 1) {
        if (this.addList.length > 0) {
          for (let i = 0; i < this.addList.length; i++) {
            if (this.addList[i].user_id === item.user_id) {
    ...
    unShuttle (item, index) {
      if (this.searchList.length > 0) {
        for (let i = 0; i < this.searchList.length; i++) {
          if (this.searchList[i].user_id === item.user_id) {
    ...
    upAddAccount () {
      ...
      if (this.activeName === 'first') {
        for (let i = 0; i < this.addList.length; i++) {
          bizList.push(this.addList[i].user_id)
    ...
    // 切换日/周/月榜单
    openOther (type) {
      let url = type === '1' ? 'CustimizeDetail' : type === '2' ? 'CustimizeDouyinArticle' : 'CustimizeDouyinVideo'
    ...
    toSearch (id) {
      ...
      this.$router.push({ name: 'ReleaseRulesDouyin', query: { id: id } })
    }
  },
  created () {
