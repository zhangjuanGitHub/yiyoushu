 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-08-03 15:30:20
-->
<template>
  <div>
    <div>
      <!-- <div class="title-time">省域 <span>{{ruleForm.date}}</span> 排行总榜</div> -->
      <div class="title-time">{{this.regionList}} <span>{{ruleForm.date}}</span> 排行总榜</div>
      <div class="from-box">
        <el-form :inline="true"
                 ref="ruleForm"
                 :model="ruleForm">
...
export default {
  ...
  methods: {
    ...
    geListData () {
      ...
      this.$http.post(this.$api.zfToutiaoListData, this.ruleForm)
        .then(res => {
    ...
    // getListDataWb () {
    //   console.log(this.ruleFormWb.functionType)
    //   this.ruleFormWb.functionType = this.ruleForm.functionType.toString()
    //   this.ruleFormWb.date = this.ruleForm.date
    //   console.log(this.ruleFormWb.functionType)
    //   console.log(this.ruleFormWb)
    //   // var par = {
    //   //   // types: '1',
    //   //   rankingType: '1', // 榜单类型 1：日榜 2：周榜 3：月榜
    //   //   date: '2021-05-19', // 时间
    ...
    //   //   pageSize: '10'
    //   // }
    //   this.$http.post(this.$api.zfWbListData, this.ruleFormWb)
    //     .then(res => {
    //       if (res.data.data.content.length > 0) {
    //         this.tableDataWb.push(...res.data.data.content)
    //         this.limitCount = res.data.data.limitCount
    //         if (res.data.data.content.length < 10) {
    //           this.dataShow = false
    //         } else {
    ...
    //           this.pageShow = false
    //         } else {
    //           if (this.tableDataWb.length >= this.limitCount) {
    //             this.pageShow = true
    //           } else {
    //             this.pageShow = false
    //           }
    ...
    getRegion () {
      this.regionList = ''
      if (this.ruleForm.districts.length > 0) {
        this.regionList = ''
        for (var i = 0; i < this.ruleForm.districts.length; i++) {
          this.regionList += this.ruleForm.districts[i]
        }
