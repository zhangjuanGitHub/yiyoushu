      ...
      <div class="list-main flex-cloumn-ll">
          ...
          <el-form :inline="true"
            ...
            <el-form-item label="权重设置"
              ...
              <el-radio v-model="ruleForm.type"
                        label=""
                        @change="changeDateType">全部</el-radio>
              <el-radio v-model="ruleForm.type"
                        label="1"
                        @change="changeDateType">微信</el-radio>
              <el-radio v-model="ruleForm.type"
                        label="2"
                        @change="changeDateType">微博</el-radio>
            </el-form-item>
            <el-form-item label="榜单周期"
                          prop="publishTime"
                          class="sear-date">
        ...
        <div class="list-main-list"
          ...
          <div class="main-list-box"
               ...
               :class="[{ 'is-todelete': comfirmDel }, { 'cursor': item.status===2 } ]"
            ...
            <div class="flex-bwt-center top_all">
              <div class="main-list-up flex-ali-center left-head-img">
                ...
                <img src="@/assets/images/assess/wx2.png"
                     v-if="item.type=='1'">
                <img src="@/assets/images/assess/wb2.png"
                     v-else>
                <div class="main-list-name cursor">
                  <p class="nickname-p"
                     v-if="item.type=='1'">微信</p>
                  <p class="nickname-p"
                     v-else>微博</p>
              ...
              <div class="top-status">
                <span class=""
                      v-if="item.status=='0'">榜单制作未开始</span>
                <span class="sta_2"
                      v-if="item.status=='1'">榜单制作进行中</span>
                <span class="sta_1"
                      v-if="item.status=='2'">榜单制作完成 </span>
                <span class="sta_3"
                      v-if="item.status=='-1'">榜单制作失败</span>
            ...
            <div class="main-list-time2">
              ...
              <p><img src="@/assets/images/assess/1.png">影响力:<span>{{item.influence}}</span></p>
              <p><img src="@/assets/images/assess/2.png">传播力:<span>{{item.spread}}</span></p>
              <p><img src="@/assets/images/assess/3.png">引导力:<span>{{item.guide}}</span></p>
              <!-- <p>粉丝数:<span class="sp">{{item.followersCount}}</span></p> -->
            </div>
            <div class="main-list-time">
              <p>榜单周期：<span>{{item.weightStartTime}}</span> ~ <span>{{item.weightEndTime}}</span></p>
              <p>榜单创建时间：<span>{{item.createTime}}</span></p>
              <p>生成榜单时间：<span>{{item.updateTime}}</span></p>
            </div>
        ...
        <div class="pagination-wrap flex-all-center" v-if="this.total2>12">
...
export default {
  ...
  methods: {
    ...
    hdToAnalyse (item) {
      ...
      // }
      // if (item.analyseScore > 0) {
      //   // this.$router.push({ name: 'JusticeDetailWx', query: { id: item.id } })
      //   this.$router.push({ name: 'JusticeDetailWb', query: { id: item.id } })
      // }
      if (item.status == '2') {
        if (item.type == '1') {
          this.$router.push({ name: 'JusticeDetailWx', query: { id: item.id } })
    ...
    toAnalyse (item) {
      ...
      let routeName = this.ruleForm.type == '1' ? 'WeekDetailWb' : 'WeekDetailWb'
    ...
    confirmDelete () {
      ...
      let ids2 = ids.join(',')
      // console.log(ids2)
      this.$http.get(this.$api.rankingListDelete, { params: { ids: ids2 } })
        .then(res => {
          ids.forEach((item) => {
    ...
    getTaskList () {
      ...
      // }
      if (this.ruleForm.publishTime.length > 0) {
        this.ruleForm.startTime = this.ruleForm.publishTime[0]
        this.ruleForm.endTime = this.ruleForm.publishTime[1]
      }
      this.$http.post(this.$api.rankingList, this.ruleForm)
        .then(res => {
