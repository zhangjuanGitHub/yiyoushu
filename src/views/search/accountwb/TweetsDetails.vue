 ...
 * @LastEditTime: 2021-05-25 14:08:08
-->
<template>
      ...
      <div class="search-analyse-up">
        <div class="nav-breadcrumb flex-ali-center">
          <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{ name: 'HistoryTweetsWb', query: { id: this.accountId} }">历史推文</el-breadcrumb-item>
            <el-breadcrumb-item :to="{ name: 'TweetsDetails' }">文章详情</el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        <div class="search-analyse-art">
          <div class="wb-detail-up flex">
            <div class="wb-detail-ava">
              <img :src="accountMsg.profile_image_url" alt="">
            </div>
            <div class="wb-detail-info">
              <div class="wb-detail-msg flex-ali-center">
                ...
                <img v-if="accountMsg.verified && accountMsg.verified_type === 0 && accountMsg.verified_type_ext === 1"
                     src="@/assets/images/search/v_1.png" alt="">
                <img v-else-if="accountMsg.verified && accountMsg.verified_type === 0 && accountMsg.verified_type_ext === 0"
                     src="@/assets/images/search/v_2.png" alt="">
                <img v-else-if="accountMsg.verified"
                     src="@/assets/images/search/v_3.png" alt="">
              </div>
              <div class="wb-detail-cont">
                <p class="lin-clamp-3 cursor" @click="targetUrl(articleMsg.url)">{{articleMsg.raw_text}}
          ...
          <div class="wb-detail-down flex-arr-center">
            <div class="flex-ali-center">
              <img src="@/assets/images/search/de_1.png" alt="" style="margin-top: -3px">
              <p>{{articleMsg.attitudes_count}}</p>
            </div>
            <div class="left-height-wb-one"></div>
            <div class="flex-ali-center">
              <img src="@/assets/images/search/de_2.png" alt="">
              <p style="color:#F79406">{{articleMsg.comments_count}}</p>
            </div>
            <div class="left-height-wb-one"></div>
            <div class="flex-ali-center">
              <img src="@/assets/images/search/de_3.png" alt="">
              <p>{{articleMsg.reposts_count}}</p>
            </div>
          </div>
        </div>
        <div class="search-analyse-main">
          ...
          <p class="flex-all-center" v-if="isNotComment">暂无评论</p>
          <ul class="wb-ul" v-else :style="{height: ((pinglunData.length + 1)/2) *80 +'px'}">
            <li class="flex" v-for="(item, index) of pinglunData" :key="index">
              <div class="wb-comment-box flex-ali-center">
                ...
                <div>
                  ...
                  <p style="margin-top: 5px">{{item.reply_time}}</p>
                </div>
              </div>
              <p class="wb-comment-cont">{{item.raw_text}}</p>
            </li>
          </ul>
          <p class="flex-all-center" v-if="isMore" style="clear:both; padding-top:10px">
            <span class="primary cursor" @click="getComments">点击加载更多</span>
          </p>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getComments () {
      let obj = {
        ...
        mid: this.articleMsg.mid
        // mid: '4630256097692398'
        // mid: '4636543362533016'
      }
      this.$http.post(this.$api.getTweetsComment, obj)
        .then(res => {
    ...
    getHTArticle () {
      this.$http.post(this.$api.getTweetsDetails, { id: this.articleId, uid: this.accountId } )
        .then(res => {
  ...
  created () {
    this.accountId = this.$route.query.uid
