 ...
 * @LastEditors: MaiChao
 * @LastEditTime: 2021-08-04 18:07:11
-->
<template>
    ...
    <div class="search-rules-num flex-ali-center">
      ...
      <ul class="wb-rules-about" v-if="this.releaseList.length > 0">
        <li class="flex-all-center">
          <p style="background:#E9F4FF">原创头条</p>
          <p style="background:#FCF3E8">转发头条</p>
        </li>
        <li class="flex-all-center">
...
export default {
  ...
  methods: {
    ...
    // 饼图
    drawPie () {
      // console.log(this.accountToutiaoMsg.user_id)
      this.$http.post(this.$api.getWbRelease, { userId: this.accountToutiaoMsg.user_id })
        .then(res => {
    ...
    // 发文趋势
    drawBar () {
      this.$http.post(this.$api.ttTrendOfPublication, { userId: this.accountToutiaoMsg.user_id })
        .then(res => {
    ...
    // 粉丝趋势
    drawFans () {
      // console.log(console.log(this.accountToutiaoMsg.user_id))
      this.$http.post(this.$api.ttFansTrend, { userId: this.accountToutiaoMsg.user_id })
        .then(res => {
